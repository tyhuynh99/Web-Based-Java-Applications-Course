 ...
 * @author ACER
 */
public class DeleteAccCartController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            accCartBean.setPetId(petId);
            accCartBean.setAccId(accId);
            if (accCartBean.delete() == true){
                HttpSession session = request.getSession();
                ShoppingCart cart = (ShoppingCart) session.getAttribute("CART");
                String key = accId+"-"+petId;
                if (cart.removeAccCart(key) != null){
                    session.setAttribute("CART", cart);
                }
            }
        } catch (Exception e) {
            log("ERROR at DeleteAccCartController: "+e.getMessage());
        } finally{
